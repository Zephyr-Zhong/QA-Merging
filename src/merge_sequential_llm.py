...
parser.add_argument("--ranked_layers_json_path", type=str, default="./layers_analysis/layer_analysis_outputs/ranked_layers.json", help="path to ranked_layers.json generated by layer distribution analysis")
parser.add_argument("--top_k_ratio", type=float, default=0.1, help="top-k layer ratio in (0, 1]")
parser.add_argument("--init_merge_coef", type=float, default=0.1, help="Initial merge coefficient for Short-CoT model")
...
def train(args, lr, epochs, merged_train_loader, load_model_paths):
    ...
    avg_pre_merged_model = load_avg_merged_model_pre_llm(args, merge_coef=args.init_merge_coef)
    ...
    # select the top layers based on layer distribution analysis
    with open(args.ranked_layers_json_path, "r", encoding="utf-8") as f:
        ranked_records = json.load(f)
    sorted_records = sorted(
        ranked_records,
        key=lambda rec: int(rec.get("rank", 10 ** 9))
    )
    top_k = math.ceil(len(sorted_records) * args.top_k_ratio)
    selected_layer_indices = sorted(
        int(rec["layer"]) - 1
        for rec in sorted_records[:top_k]
    )

    print(f"Loaded ranked layers from: {args.ranked_layers_json_path}")
    print(f"Selected top-{args.top_k_ratio * 100:.1f}% layers ({len(selected_layer_indices)}): {selected_layer_indices}")



    print('Start training')
    for layer_idx in range(num_layers):
        if layer_idx not in selected_layer_indices:
            # calculate a cross-form merge coeffcient for non-top layers
            print(f"Layer {layer_idx} is not in the selected top layers, skipping training for this layer.")
            merged_layer, layers = load_merged_layers_llm(args=args, layer_idx=layer_idx, init_merge_weight=args.init_merge_coef)
            for layer in layers:
                layer.eval()
        else:   
    ...
    # after training
    merged_model = load_avg_merged_model_llm(args, merge_coef=args.init_merge_coef)
    for layer_idx in range(num_layers):
